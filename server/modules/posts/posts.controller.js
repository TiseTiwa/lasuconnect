...
exports.createPost = catchAsync(async (req, res, next) => {
  ...
  sendSuccess(res, {
    ...
    data: { post: formatPost(post, req.user._id) },
  });
});
...
exports.toggleLike = catchAsync(async (req, res, next) => {
  ...
  if (alreadyLiked) {
    ...
    await createAndEmit({
      io: req.io, recipientId: post.author, actorId: req.user._id,
      type: 'like', targetModel: 'Post', targetId: post._id,
      targetThumbnail: post.mediaUrls?.[0] || null,
  ...
  sendSuccess(res, {
    data: {
      comments: comments.map(c => ({
        ...c,
        isLiked: c.likes?.some(id => id.toString() === req.user._id.toString()) || false,
      })),
    },
...
exports.addComment = catchAsync(async (req, res, next) => {
  ...
  await createAndEmit({
    io: req.io, recipientId: post.author, actorId: req.user._id,
    type: 'comment', targetModel: 'Post', targetId: post._id,
    targetThumbnail: post.mediaUrls?.[0] || null,
...
exports.savePost = catchAsync(async (req, res, next) => {
  ...
  const saved = user.savedPosts?.map(id => id.toString()).includes(req.params.id);
  ...
  sendSuccess(res, { data: { saved: !saved } });
});
