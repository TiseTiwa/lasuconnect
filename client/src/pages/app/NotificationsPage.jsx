...
const TYPE_CONFIG = {
  ...
  reel_comment:     { icon: '🎬', color: '#2563EB', verb: 'commented on your reel' },
  announcement:     { icon: '📢', color: '#D97706', verb: 'New announcement' },
  live_start:       { icon: '🔴', color: '#EF4444', verb: 'went live' },
  tutoring_request: { icon: '🧑‍🏫', color: '#059669', verb: 'requested tutoring' },
};
...
const groupNotifications = (notifications) => {
  ...
  notifications.forEach(notif => {
    ...
    if (!groups.has(key)) {
      ...
      if (notif.actor && !existing.actors.find(a => a?._id === notif.actor?._id)) {
...
const AvatarStack = ({ actors = [], typeIcon, size = 44 }) => {
  ...
  const getColor = (u) => colors[(u?.username?.charCodeAt(0) || 0) % colors.length];
  const getInitials = (u) => u?.fullName?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || '?';
  const shown = actors.slice(0, 2);

  return (
    <div style={{ position: 'relative', width: size + (shown.length > 1 ? 16 : 0), height: size, flexShrink: 0 }}>
      {shown.map((actor, i) => (
        <div key={actor?._id || i} style={{
          position: 'absolute',
          left: i * 16,
          ...
          {actor?.avatarUrl
            ...
            : <span style={{ color: 'white', fontWeight: 700, fontSize: (size - 6) * 0.32, fontFamily: 'Geist, sans-serif' }}>{getInitials(actor)}</span>
          }
        </div>
      ))}
      {/* Type icon badge */}
...
const ThumbnailPreview = ({ notif }) => {
  const hasThumb = notif.type === 'like' || notif.type === 'comment' ||
                   notif.type === 'reel_like' || notif.type === 'reel_comment';
  ...
  return (
    <div style={{
      ...
      background: '#E2E8F0', flexShrink: 0, position: 'relative',
    }}>
      {notif.targetThumbnail ? (
        <img src={notif.targetThumbnail} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      ) : (
        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: isReel ? '#0F172A' : '#F1F5F9', fontSize: 18 }}>
          {isReel ? '🎬' : '📝'}
        </div>
      )}
    </div>
  ...
  return (
    <button onClick={handle} disabled={loading}
      style={{
        ...
        background: status === 'done' ? '#F1F5F9' : 'linear-gradient(135deg, #1D4ED8, #3B82F6)',
  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeft: notif.isRead ? '3px solid transparent' : `3px solid ${config.color}`,
        border: '1px solid #F1F5F9',
        borderLeft: notif.isRead ? '3px solid transparent' : `3px solid ${config.color}`,
        transition: 'background 0.15s', position: 'relative',
        marginBottom: 2,
      }}
      onMouseEnter={e => e.currentTarget.style.background = notif.isRead ? '#F8FAFC' : '#E8F1FF'}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13.5, color: '#1E293B', lineHeight: 1.5, marginBottom: 2 }}>
          <span style={{ fontFamily: 'Geist, sans-serif', fontWeight: 700 }}>
            {actors[0]?.fullName || 'Someone'}
          </span>
          {actors.length > 1 && (
            <span style={{ color: '#374151' }}> and {actors.length - 1} other{actors.length > 2 ? 's' : ''}</span>
          )}
          {' '}
        ...
        <div style={{ fontSize: 11.5, color: '#94A3B8', fontWeight: 500 }}>{timeAgo(notif.createdAt)}</div>
      </div>
      ...
      {/* Unread dot */}
      {!notif.isRead && (
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#2563EB', flexShrink: 0, marginLeft: 4 }} />
      )}
      ...
      <button
        ...
        ref={el => {
          if (el) {
            el.closest('[data-notif]')?.addEventListener('mouseenter', () => el.style.opacity = 1);
            el.closest('[data-notif]')?.addEventListener('mouseleave', () => el.style.opacity = 0);
          }
        }}
      >✕</button>
    </div>
...
const SectionHeader = ({ title }) => (
  <div style={{ padding: '16px 4px 8px', fontFamily: 'Geist, sans-serif', fontWeight: 800, fontSize: 15, color: '#0F172A' }}>
    {title}
  </div>
);
...
const NotificationsPage = () => {
  ...
  const filtered = notifications.filter(n => {
    ...
    if (activeFilter === 'like') return n.type === 'like' || n.type === 'reel_like';
    if (activeFilter === 'comment') return n.type === 'comment' || n.type === 'reel_comment';
  ...
  const earlierItems  = grouped.filter(n => !isToday(n.createdAt) && !isThisWeek(n.createdAt));
    ...
    <div style={{ paddingBottom: isMobile ? 96 : 40, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <div>
          ...
          {unreadCount > 0 && (
            <span style={{ display: 'inline-block', background: 'linear-gradient(135deg, #1D4ED8, #3B82F6)', color: 'white', borderRadius: 20, padding: '2px 10px', fontSize: 11, fontWeight: 700, marginTop: 4 }}>
              {unreadCount} new
            </span>
          )}
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {unreadCount > 0 && (
            <button onClick={handleMarkAllRead}
              style={{ padding: isMobile ? '6px 10px' : '7px 12px', borderRadius: 8, border: 'none', background: '#EFF6FF', color: '#2563EB', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", whiteSpace: 'nowrap' }}>
              ✓ {isMobile ? 'All read' : 'Mark all read'}
            </button>
          )}
          {notifications.length > 0 && (
            <button onClick={handleClearAll}
              style={{ padding: '6px 10px', borderRadius: 8, border: 'none', background: '#FEF2F2', color: '#EF4444', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
              🗑️
            </button>
          )}
        </div>
      ...
      {/* Filter tabs */}
      <div style={{ display: 'flex', gap: 5, marginBottom: 4, overflowX: 'auto', scrollbarWidth: 'none', flexWrap: isMobile ? 'nowrap' : 'wrap', paddingBottom: isMobile ? 4 : 0 }}>
        {FILTERS.map(f => (
          <button key={f.id} onClick={() => setActiveFilter(f.id)}
            style={{
              padding: isMobile ? '5px 10px' : '6px 14px', borderRadius: 20, flexShrink: 0,
              border: activeFilter === f.id ? 'none' : '1.5px solid #E2E8F0',
              background: activeFilter === f.id ? '#0F172A' : 'white',
      ...
      {loading && (
        <div>
          {[1,2,3,4,5].map(i => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 14, background: 'white', border: '1px solid #F1F5F9', marginBottom: 2 }}>
              <div style={{ width: 46, height: 46, borderRadius: '50%', background: '#E2E8F0', flexShrink: 0, animation: 'pulse 1.5s ease infinite' }} />
              <div style={{ flex: 1 }}>
                <div style={{ height: 13, background: '#E2E8F0', borderRadius: 6, width: '55%', marginBottom: 8, animation: 'pulse 1.5s ease infinite' }} />
                <div style={{ height: 11, background: '#E2E8F0', borderRadius: 6, width: '20%', animation: 'pulse 1.5s ease infinite' }} />
              </div>
              <div style={{ width: 44, height: 44, borderRadius: 8, background: '#E2E8F0', animation: 'pulse 1.5s ease infinite' }} />
            </div>
          ))}
        </div>
      )}
          ...
          <div>
            ...
            {hasMore && (
              <button
                ...
                style={{ width: '100%', padding: 12, border: '1.5px solid #E2E8F0', borderRadius: 12, background: 'white', fontSize: 13, fontWeight: 600, color: '#374151', cursor: 'pointer', marginTop: 12, fontFamily: "'DM Sans', sans-serif" }}>
                {loadingMore
            ...
            {!hasMore && grouped.length > 5 && (
              <div style={{ textAlign: 'center', fontSize: 13, color: '#94A3B8', padding: '20px', fontWeight: 600 }}>
